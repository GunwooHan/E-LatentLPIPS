...
def translate4d(tx, ty, tz, ta, **kwargs):
    return matrix(
        [1, 0, 0, 0, tx],
        [0, 1, 0, 0, ty],
        [0, 0, 1, 0, tz],
...
def scale4d(sx, sy, sz, sa, **kwargs):
    return matrix(
        [sx, 0, 0, 0, 0],
        [0, sy, 0, 0, 0],
        [0, 0, sz, 0, 0],
...
def rotate4d(u, v, theta, **kwargs):
    """
    Create a 4D rotation matrix to rotate around the plane defined by u and v.

    Args:
        u (torch.Tensor): First 4D vector defining the rotation plane.
        v (torch.Tensor): Second 4D vector defining the rotation plane.
        theta (torch.Tensor): Rotation angle in radians.

    Returns:
        torch.Tensor: 5x5 rotation matrix for homogeneous coordinates.
    """
    u = u / torch.norm(u)
    v = v - torch.dot(u, v) * u
    v = v / torch.norm(v)
    ...
    rotation_matrix = cos_theta * torch.eye(4, device=u.device) + \
                      sin_theta * (uvT - vuT) + \
                      cc * (uuT + vvT)

    # Convert to homogeneous coordinates (5x5 matrix)
    rotation_matrix_homogeneous = torch.eye(5, device=u.device)
    rotation_matrix_homogeneous[:4, :4] = rotation_matrix
...
class AugmentPipe(torch.nn.Module):
    ...
    def forward(self, images, label_images, debug_percentile=None):
        ...
        if G_inv is not I_3:
            ...
            label_images = torch.nn.functional.pad(input=label_images, pad=[mx0, mx1, my0, my1], mode='reflect')
            G_inv = translate2d((mx0 - mx1) / 2, (my0 - my1) / 2) @ G_inv

            # Upsample.
            images = upfirdn2d.upsample2d(x=images, f=self.Hz_geom, up=2)
            label_images = upfirdn2d.upsample2d(x=label_images, f=self.Hz_geom, up=2)
            ...
            label_images = grid_sample_gradfix.grid_sample(label_images, grid)

            # Downsample and crop.
            images = upfirdn2d.downsample2d(x=images, f=self.Hz_geom, down=2, padding=-Hz_pad * 2, flip_filter=True)
            label_images = upfirdn2d.downsample2d(x=label_images, f=self.Hz_geom, down=2, padding=-Hz_pad * 2,
                                                  flip_filter=True)
        ...
        v = misc.constant(np.asarray([1, 1, 1, 1, 0]) / np.sqrt(3), device=device)  # Luma axis.
        if self.lumaflip > 0:
        ...
        if C is not I_4:
            ...
            label_images = label_images.reshape([batch_size, num_channels, height * width])

            if num_channels == 3:
                images = C[:, :3, :3] @ images + C[:, :3, 3:]
                label_images = C[:, :3, :3] @ label_images + C[:, :3, 3:]
            elif num_channels == 1:
                C = C[:, :3, :].mean(dim=1, keepdims=True)
                images = images * C[:, :, :3].sum(dim=2, keepdims=True) + C[:, :, 3:]
                label_images = label_images * C[:, :, :3].sum(dim=2, keepdims=True) + C[:, :, 3:]
            elif num_channels == 4:
                images = C[:, :4, :4] @ images + C[:, :4, 4:]
                label_images = C[:, :4, :4] @ label_images + C[:, :4, 4:]
            else:
        ...
        if self.imgfilter > 0:
            ...
            # Apply filter.
            p = self.Hz_fbank.shape[1] // 2
            label_images = label_images.reshape([1, batch_size * num_channels, height, width])
            label_images = torch.nn.functional.pad(input=label_images, pad=[p, p, p, p], mode='reflect')
            label_images = conv2d_gradfix.conv2d(input=label_images, weight=Hz_prime.unsqueeze(2),
                                                 groups=batch_size * num_channels)
            label_images = conv2d_gradfix.conv2d(input=label_images, weight=Hz_prime.unsqueeze(3),
                                                 groups=batch_size * num_channels)
            label_images = label_images.reshape([batch_size, num_channels, height, width])
        ...
        if self.noise > 0:
            ...
            noise = torch.randn([batch_size, num_channels, height, width], device=device)
            images = images + noise * sigma
            label_images = label_images + noise * sigma
        ...
        if self.cutout > 0:
            ...
            images = images * mask
            label_images = label_images * mask
...
if __name__ == '__main__':
    ...
    transform = AugmentPipe(
        xflip=1 if args.blit else 0,
        rotate90=1 if args.blit else 0,
        xint=1 if args.blit else 0,
        scale=1 if args.geom else 0,
        rotate=1 if args.geom else 0,
        aniso=1 if args.geom else 0,
        xfrac=1 if args.geom else 0,
        brightness=1 if args.color else 0,
        contrast=1 if args.color else 0,
        # lumaflip=1 if args.color else 0,
        # hue=1 if args.color else 0,
        saturation=1 if args.color else 0,
    ...
    torchvision.utils.save_image(torch.cat(outputs, dim=0), "aug_comp.png", nrow=2, normalize=True)
